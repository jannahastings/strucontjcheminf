...
Chemoinformatics solutions have been developed which facilitate classification of sets of chemical entities by automatic methods.  Such automatic classification systems are in common use in many areas of chemistry including drug discovery.  Almost since the inception of chemoinformatics, chemical structures have been formalized as graphs in computer systems. Yet, standard algorithms for automated classification tend not to be easily amenable to operation on arbitrary graph-based data structures, so the graphs are often transformed -- preferably through inexpensive algorithms -- into characteristic \textit{features} or \textit{descriptors}, which then in turn form the input to classification algorithms. 

As defined by Todeschini and Consonni\cite{Todeschini2000}, a molecular descriptor is the final result of a logical and mathematical procedure which transforms chemical information encoded within a symbolic representation of a molecule into a useful number (calculated descriptors), or, the result of standardized experiments (experimental descriptors). Among calculated descriptors, if we focus on structural features, molecular fingerprints are a special form of descriptors which are defined as strings of boolean values, in which each bit represents a feature.  In the most common types of fingerprint, a feature could be either a pre-defined substructure or a random substructure mapped by hashing algorithms.

Features are then passed to hierarchy construction algorithms such as \textit{hierarchical similarity clustering} \cite{Adamson1991,barnard1992}.  In these methods, similarity between compounds is computed on a pairwise basis, and compounds are then grouped together in clusters which represent high mutual similarity along a particular dimension.  Pairwise similarity is usually calculated based on a comparison of calculated descriptors or fingerprints for the molecule, rather than on the molecular graph directly.  Either agglomerative or divisive methods could be used. In agglomerative clustering, all instances are initially assigned to their own class, and a recursive merge is performed by the algorithm, which chooses two nearest classes to be merged in each step, until all classes are merged into one. Divisive clustering is performed in the opposite fashion: assigning all instances to one class initially, and performing a split instead of a merge in each recursive step.  

The results of such methods are additionally dependent both on the algorithms used to convert the molecular graph into fingerprints or collections of features, and on the calculation of similarity between resulting feature sets. There are thus many different algorithms for computing similarity measures between compounds and for aggregating compounds into clusters based on pairwise similarity measures, leading to arbitrarily many different classification hierarchies as their result, even given the same compound collection as input.  A limitation of the similarity-based hierarchical clustering analysis is thus that each resulting node in the hierarchy represent a class with an unclear meaning (and there is no formal definition for such nodes).  Changes in input data (or structures) may result in a different hierarchical tree with different intermediate nodes, making it very difficult to compare results across datasets even when the same clustering and fingerprinting algorithms are used in constructing the output. Another limitation is that most feature sets on which similarity measures are calculated, in order to be efficiently computable, represent only a subset of the total features of the molecules concerned, and local paths through the structures predominate over the (more expensive) overall molecular structure. Thus, molecules may turn out to appear quite similar according to such an algorithm (due, perhaps, to a predominance of similar parts) while displaying rather different structures overall.  This problem is exacerbated for molecules of high structural regularity (e.g. polycyclic carbon compounds).  Nevertheless, similarity landscapes are of paramount importance in reducing the complexity and understanding the features of large collections of compounds. 
 
%To what extent are / should SMARTS and R-GROUPS be relevant here?

Beyond algorithmic approaches, statistical machine learning approaches have become increasingly important in the classification of chemicals, albeit with slightly different application.  Structure-activity relationship (SAR) studies have become one of the hottest fields where machine learning approaches are applied. Supervised methods, such as Bayesian classifiers, decision trees and support vector machines,  are employed to classify compounds into positive or negative groups for a particular functional activity class.  However, these approaches, resulting as they do in binary output, do not immediately lend themselves to the complex hierarchical classification task.  Supervised machine learning for the prediction of chemical class membership based on an existing hierarchy is an interesting option but it necessitates a simplification of the possible hierarchy to reduce the number of classes to levels which even the most sophisticated multi-class classification technique can accomplish, and furthermore requires a reasonably large training set of chemicals which are already classified.  Although existing databases like ChEBI and MeSH~\cite{meshUrl} could act as training sets for such an endeavor, the size of these data is still a tiny fraction of the enormous chemical space, and the problem is further complicated by the fact that the leaf nodes of such classification trees normally contain few structures. 
...
%% Table 1
\subsection*{Table 1 -- table title}
\begin{table}[h!]
\centering
\caption{DL-safe rules example}\label{tab:DL-safe-example}
\rowcolors{1}{tableShade}{white}
